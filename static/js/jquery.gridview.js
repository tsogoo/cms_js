(function($) {
    ...
    $.fn.selectIds = function(command) {
        ...
        $('input:checked', this).each(function() {
            switch(command) {
                case 'publish':
                    src = $(this).parent().parent().children('.i_state').children('a').children('img').attr('src');
                    if(src.match(/\d/) == 0)
                        ids.push($(this).val());
                    break;
                case 'unpublish':
    ...
    $.fn.initGridView = function(d, options) {
        ...
        form.append(sortField, sortBy, currentPage, command, ids);
        ...
        form.bind('submit', function() {
            $.ajax({
                ...
                success : function(data) {
                    that.createGridView(data, options)
                },
                dataType : "json"
            })
            return false
    ...
    $.fn.createGridView = function(d, options) {
        ...
        var options = _.extend( defopt, options);
        var elems = _.extend(defaults, options);
        
        $(options.filter_form + ' input[name="sort"]').val(d.sort)
        $(options.filter_form + ' input[name="sort"]').val(d.sort)
        var s;
        ...
        if(d.pager.currentPage + 1 < pages) {
            ...
            row = $(document.createElement('td')).attr({
                colspan : d.attributes.length + 1
        ...
        for(var i = 1; i <= pages; i++) {
            if(i - 1 == d.pager.currentPage) {
                pageNum = $(document.createElement('span')).html(i)
            } else {
                pageNum = $(document.createElement('a')).attr('href', '#').html(i)
            }
        ...
        $('.li_commands a').bind('click', function() {
            if($('#dtable tbody td input:checked').length > 0) {
                ...
                var ids = $('#dtable tbody', that).selectIds(command);
                if(ids != '') {
                    confirmation = elems.changeMsg;
                    if(command == 'delete')
                        confirmation = elems.delMsg;
        ...
        $('#dtable th a').bind('click', function() {
            ...
            if($(options.filter_form + ' #sorting').val() == sorting) {
                sortby = ($(options.filter_form + ' #sortby').val($(options.filter_form + ' #sortby').val() * (-1) + 1));
            } else {
                $(options.filter_form + ' #sorting').val(sorting);
                $(options.filter_form + ' #sortby').val(0)
            }
            $.ajax({
                type : 'POST',
                url : $(options.filter_form).attr('action'),
        ...
        $('.i_state a').bind('click', function() {
            ...
            $.ajax({
                ...
                data : ( {
                    ...
                    'state' : state,
                    'command' : 'ajaxpub',
                    YII_CSRF_TOKEN : options.CSRF_token
                }),
                success : function(data) {
                    $('#dtable tbody').changeStateByOne(data, dragged)
                },
                dataType : "json"
            });
            return false
        ...
        if(d.sorting[0] == 'i_ordering') {
            $("#dtable").tableDnD({
                ...
                onDrop : function(table, row) {
                    ...
                    if($('td input:checkbox', row).val() == $('tbody tr:last td input:checkbox', table).val() && $('tbody #nextPage', table).length) {/*next page*/
                        size = d.pager.size;
                        pag = d.pager.currentPage;
                        data = $('td > input:checkbox', row).val() + ',' + $('.dragHandle > img', row).attr('alt');
                        up = 0;
                    } else if($('td input:checkbox', row).val() == $('tbody tr:first td input:checkbox', table).val() && $('tbody #prevPage', table).length) {/*prev page*/
                        size = d.pager.size;
                        pag = d.pager.currentPage;
                        data = $('td > input:checkbox', row).val() + ',' + $('.dragHandle > img', row).attr('alt');
                        up = 1;
                    } else {
                        data = $("#dtable tbody").orderValues();
                        size = '';
                        pag = '';
                        up = '';
                        sortby = '';
                    ...
                    if(data != '')
                        $.ajax({
                            ...
                            success : function(data) {
                                if(data == 1) {
                                    $('tbody', that).afterDrop()
                                } else if(data == 0) {alert('Ð°Ð»Ð´Ð°Ð°')
                                } else {
                                    that.createGridView(data, options)
                                }
                            },
    ...
    $.fn.initColumn = function(d, col, i, j) {
        if(d.attributes[j].name == 'id') {
            ...
            if(d.attributes[j].template) {
                ...
                template = eval('d.data[i].a.' + d.attributes[j].name)
            col.html(template);
        }
        return col
    }
    ...
    $.fn.updateOrderValues = function() {
        ...
        var j = 0;
        $('tr', this).not("#nextPage,#prevPage").each(function() {
            $('.dragHandle > img', this).attr('alt', values[j]);
            j++;
        });
    }
